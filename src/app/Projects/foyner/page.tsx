...
const tech = ["Next.js", "React", "TypeScript", "Tailwind CSS", "Photography", "Photo Editing", "Canon EOS 700D", "iPhone 14–16", "GoPro HERO12", "Responsive Design", "Vercel"];
    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16 pt-36 md:pt-44">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          ...
          <div className="flex flex-wrap gap-3 lg:justify-end"><a href="https://www.foyner.no/" target="_blank" rel="noopener noreferrer" className="rounded-full bg-white px-6 py-3 font-semibold text-black transition hover:scale-105">{no ? "Se nettsiden ↗" : "Live website ↗"}</a><a href="https://github.com/PMariusf/foyner" target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/20 bg-white/[0.06] px-6 py-3 font-semibold transition hover:bg-white/[0.12]">GitHub ↗</a></div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-14"><div className="grid gap-5 md:grid-cols-3"><div className="rounded-3xl border border-white/15 bg-white/[0.05] p-7"><p className="text-xs uppercase tracking-[0.28em] text-white/55">{no ? "Min rolle" : "My role"}</p><p className="mt-4 text-xl font-semibold">Frontend · UI/UX · {no ? "Fotografi" : "Photography"}</p></div><div className="rounded-3xl border border-white/15 bg-white/[0.05] p-7"><p className="text-xs uppercase tracking-[0.28em] text-white/55">{no ? "Prosjekttype" : "Project type"}</p><p className="mt-4 text-xl font-semibold">{no ? "Foto & portfolio" : "Photography & portfolio"}</p></div><div className="rounded-3xl border border-white/15 bg-white/[0.05] p-7"><p className="text-xs uppercase tracking-[0.28em] text-white/55">{no ? "Fokus" : "Focus"}</p><p className="mt-4 text-xl font-semibold">Photography · Responsive · UX</p></div></div></section>

      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-16 lg:grid-cols-2">
        <article className="rounded-4xl border border-white/15 bg-white/[0.045] p-8 md:p-10"><p className="text-xs font-semibold uppercase tracking-[0.3em] text-amber-300/70">01 · {no ? "Fotografiet" : "The photography"}</p><h2 className="mt-4 text-3xl font-bold">{no ? "Fra kamera til ferdig portfolio" : "From camera to finished portfolio"}</h2><p className="mt-5 text-base leading-8 text-white/75">{no ? "Fotografiene på Foyner er mine egne. Jeg fotograferer med Canon EOS 700D, iPhone 14–16 og GoPro HERO12, avhengig av motiv og situasjon. Prosjektet viser dermed hele den kreative prosessen min: fra fotografering og bildeutvalg til visuell presentasjon og ferdig nettside." : "The photographs featured on Foyner are my own. I shoot with a Canon EOS 700D, iPhone 14–16 and GoPro HERO12 depending on the subject and situation. The project therefore shows my full creative process, from photography and image selection to visual presentation and the finished website."}</p></article>
